...
```bash
docker build -f docker/Dockerfile -t kaimaemae-api:latest .
docker run --rm -p 8000:8000 kaimaemae-api:latest
```
...
- `KAIMAEMAE_MODELS_DIR` where the model files live. Defaults to `/app/models`
  inside the image, already set in the Dockerfile.
- `KAIMAEMAE_CORS_ORIGINS` comma separated allowed origins. Set this to your
  frontend URL when you wire up the UI, for example
  `KAIMAEMAE_CORS_ORIGINS=http://localhost:5173`.
...
docker build --platform linux/amd64 -f docker/Dockerfile -t kaimaemae-api:latest .

# 2. Create an ECR repo once.
aws ecr create-repository --repository-name kaimaemae-api

# 3. Log in, tag, and push (replace ACCOUNT and REGION).
aws ecr get-login-password --region REGION \
  | docker login --username AWS --password-stdin ACCOUNT.dkr.ecr.REGION.amazonaws.com
docker tag kaimaemae-api:latest ACCOUNT.dkr.ecr.REGION.amazonaws.com/kaimaemae-api:latest
docker push ACCOUNT.dkr.ecr.REGION.amazonaws.com/kaimaemae-api:latest
```

Then point an ECS task definition at that image, map container port 8000, and set
`KAIMAEMAE_CORS_ORIGINS` to your frontend origin. The health check path for an ECS
