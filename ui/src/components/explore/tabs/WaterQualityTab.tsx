...
const COLUMNS: TableColumn<WQRow>[] = [
  ...
  { key: "month", label: "Month", numeric: true, width: "70px",
    render: (v) => MONTH_LABELS[Number(v) - 1] ?? String(v) },
  { key: "latitude", label: "Lat", numeric: true, width: "80px",
    render: (v) => typeof v === "number" ? v.toFixed(4) : String(v) },
  { key: "longitude", label: "Lon", numeric: true, width: "90px",
    render: (v) => typeof v === "number" ? v.toFixed(4) : String(v) },
];
...
export function WaterQualityTab() {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((r) => (unsafeFilter === "unsafe" ? r.unsafe === 1 : r.unsafe === 0));
  }, [data, unsafeFilter]);
  ...
  const monthData = useMemo(() => {
    if (!data.length) return [];
    const months = MONTH_LABELS.map((m, i) => ({ month: m, rate: 0, total: 0, unsafe: 0, idx: i }));
    data.forEach((r) => {
    ...
    months.forEach((m) => {
      if (m.total > 0) m.rate = parseFloat(((m.unsafe / m.total) * 100).toFixed(2));
    });
    return months;
  }, [data]);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div>
          <h1 className="text-xl font-bold text-ocean-900">Beach Water Quality</h1>
          <p className="text-sm text-ocean-500">
            Hawaii DOH Clean Water Branch · enterococcus CFU/100 mL · 1990 – 2024
          </p>
        </div>
      </div>
      ...
      {stats && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <StatCard label="Total samples" value={stats.total.toLocaleString()} />
          <StatCard label="Beaches" value={String(stats.beaches)} sub="unique sites" />
          <StatCard label="Unsafe samples" value={stats.unsafeCount.toLocaleString()} sub="> 130 CFU" />
          <StatCard label="Unsafe rate" value={`${stats.unsafeRate}%`} sub="island-wide" />
        </div>
      )}
      ...
      {data.length > 0 && (
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-ocean-100">
            ...
            <p className="mb-4 text-xs text-ocean-400">min 30 samples · % unsafe (above 130 CFU)</p>
              ...
              >
                ...
                <Bar dataKey="rate" radius={[0, 4, 4, 0]} maxBarSize={16}>
                  {topBeachesData.map((entry, i) => (
                    <Cell
                      key={i}
                      fill={entry.rate > 20 ? "#DD5C3C" : entry.rate > 8 ? "#E8BE5C" : "#3FA8D4"}
                    />
                  ))}
                </Bar>
              </BarChart>
          ...
          <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-ocean-100">
            ...
            <p className="mb-4 text-xs text-ocean-400">
              % of samples exceeding 130 CFU — wet season (Nov–Mar) spikes are visible
            </p>
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={monthData} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
                ...
                <Tooltip
                  ...
                  contentStyle={{ fontSize: 12, borderRadius: 8, border: "1px solid #A2DBEF" }}
                />
                <Bar dataKey="rate" fill="#3FA8D4" radius={[4, 4, 0, 0]} maxBarSize={32} />
              </BarChart>
            </ResponsiveContainer>
            ...
            <div className="mt-5 border-t border-ocean-50 pt-4">
              ...
              <div className="space-y-1.5 text-xs text-ocean-600">
                <div><strong>date</strong> — sample collection date (YYYY-MM-DD)</div>
                <div><strong>location_name</strong> — beach name from DOH records</div>
                <div><strong>enterococcus</strong> — measured bacteria count (CFU/100 mL)</div>
                <div><strong>unsafe</strong> — 1 if enterococcus &gt; 130 BAV threshold, else 0</div>
                <div><strong>month</strong> — calendar month (seasonality proxy)</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-ocean-100">
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-ocean-400">45,553 rows · sortable, searchable, paginated</p>
          </div>
        </div>
        <DataTable
          columns={COLUMNS}
